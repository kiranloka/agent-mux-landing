...
import { Users, LayoutPanelLeft, Rocket, Check, Folder, Code2, Terminal, Bot, Cpu, Boxes } from 'lucide-react';
...
const WaitlistForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 relative z-10">
      ...
      <p className={`min-h-[1.25rem] text-xs font-mono ${status === "error" ? "text-red-300" : status === "success" ? "text-green-300" : "text-zinc-500"}`}>
        {message || "Leave your name and email for early access."}
      </p>
    </form>
  );
};
...
const cursorLogoUrl = new URL("./assets/cursor-cube-dark.svg", import.meta.url).href;
const deepSeekLogoUrl = new URL("./assets/deepseek-icon.svg", import.meta.url).href;
const copilotLogoUrl = new URL("./assets/copilot-icon.svg", import.meta.url).href;
const openCodeLogoUrl = new URL("./assets/opencode-icon.svg", import.meta.url).href;

const LogoImage = ({ src, className = "w-5 h-5 object-contain" }: { src: string; className?: string }) => (
...
const OrbitingAgents = () => {
  ...
  const ring1 = [
    { name: 'Codex', icon: <SiOpenai className="w-5 h-5" />, color: 'text-white', bg: 'bg-teal-500/10', border: 'border-teal-500/30', glow: 'rgba(45,212,191,0.25)' },
    { name: 'Gemini', icon: <SiGooglegemini className="w-5 h-5" />, color: 'text-white', bg: 'bg-sky-500/10', border: 'border-sky-500/30', glow: 'rgba(56,189,248,0.25)' },
  ...
  const ring2 = [
    { name: 'Claude', icon: <SiClaude className="w-5 h-5" />, color: 'text-white', bg: 'bg-orange-500/10', border: 'border-orange-500/30', glow: 'rgba(251,146,60,0.25)' },
          ...
          return (
            ...
            >
              <div className="animate-orbit-inner-reverse">
                ...
                >
                  <div
                    className={`w-11 h-11 rounded-full border ${agent.border} ${agent.bg} ${agent.color} flex items-center justify-center backdrop-blur-sm cursor-default`}
                    style={{ boxShadow: `0 0 16px ${agent.glow}, 0 2px 8px rgba(0,0,0,0.4)` }}
                    title={agent.name}
                  >
                    {agent.icon}
                  </div>
      ...
      <main className="w-full">
        ...
        <section id="pricing" aria-label="Pricing" className="py-24 border-t border-zinc-800/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            <div className="bg-zinc-900 border border-green-500/40 relative rounded-xl p-8 flex flex-col overflow-hidden">
              <div className="absolute inset-0 bg-green-400/5 pointer-events-none"></div>

              <div className="flex items-center justify-between mb-1 relative z-10">
                <h3 className="text-white font-mono font-bold text-lg">cloud</h3>
                <span className="bg-green-400/10 text-green-400 border border-green-400/20 text-xs font-mono px-2.5 py-1 rounded">coming soon</span>
              </div>
              ...
              <div className="space-y-4 mb-8 flex-grow relative z-10">
                <div className="flex items-center gap-3 text-sm text-zinc-300">
                  <Check className="text-purple-400 w-4 h-4 flex-shrink-0" />
